...
class ColorpaletteAPIClientError(Exception):
...
class ValidationError(ColorpaletteAPIClientError):
...
class ColorpaletteAPIClient:
    ...
    VALIDATION_RULES = {
        ...
        "scheme": {"type": "string", "required": False, "default": "triade"},
        "variation": {"type": "string", "required": False, "default": "default"},
        "count": {"type": "integer", "required": False, "min": 1, "max": 16, "default": 5},
        "distance": {"type": "number", "required": False, "min": 0, "max": 1, "default": 0.5},
        "addComplement": {"type": "boolean", "required": False, "default": False},
        "webSafe": {"type": "boolean", "required": False, "default": False}
    }
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the ColorpaletteAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises ColorpaletteAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise ColorpaletteAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[ColorpaletteAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns ColorPaletteGeneratorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises ColorpaletteAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise ColorpaletteAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise ColorpaletteAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise ColorpaletteAPIClientError(f"Request failed: {str(e)}")
